...
#### 포맷
앞에서 정의한 토큰과 그룹을 조합해서 최종적인 포맷을 만든다. 포맷은 하나 이상 정의할 수 있다. 실재 로그를 파싱할 때는 앞에서 부터 시도하여 정규식 매칭에 성공하면 그것을 저장하게 된다. 따라서 **자주 나오는 로그 형태를 먼저 정의하는 것이 파싱 속도를 올리기위해 중요**하다.
