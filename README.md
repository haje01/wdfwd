...
로그 안에 JSON형태의 필드가 있고, 이것을 파생해서 보내야 한다면 트랜스폼 함수를 사용한다.
...
트랜스폼된 결과는 다른 트랜스폼 함수를 통해 변경될 수 있다.

위의 예에서 'A'는 중첩된 dictionary 형의 값을 가지고 있다. 이것은 Elasticsearch등의 검색에는 좋지 않다. 이것을 펼쳐주기 위해서 `jsondata`를 다음과 같이 정의한다.

    jsondata: ['{.*}', 'ravel(json(_))']

이렇게 하면 아래와 같이 펼쳐진 결과로 된다.
...
이를 방지하기 위해 `prefix`를 사용하면,

    jsondata: ['{.*}', 'prefix(lower(ravel(json(_))), 'dat')']

다음과 같이 충돌을 방지할 수 있다.

    {'date': '2016-03-19', 'dat-date': '2001-01-01'}
